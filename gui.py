...
def phase_vocoder(a, b, fade_out, fade_in):
    """
    相位声码器，用于音频信号的相位对齐和混合。
    
    Args:
        a (torch.Tensor): 输入信号1。
        b (torch.Tensor): 输入信号2。
        fade_out (torch.Tensor): 淡出窗口。
        fade_in (torch.Tensor): 淡入窗口。
...
class Harvest(multiprocessing.Process):
    """
    多进程音频处理类，用于提取音频信号的基频（F0）。
    
    Args:
        inp_q (multiprocessing.Queue): 输入队列。
        opt_q (multiprocessing.Queue): 输出队列。
    """
...
if __name__ == "__main__":
    ...
    class GUIConfig:
        """
        GUI配置类，用于存储和管理所有界面配置参数。

        配置项说明:
            pth_path (str): 模型权重文件路径，默认为空字符串。
            index_path (str): 特征索引文件路径，默认为空字符串。
            pitch (int): 音高调整参数(半音)，默认为0。
            formant (float): 共振峰调整参数，默认为0.0。
            sr_type (str): 采样率类型("sr_model"或"sr_device")，默认为"sr_model"。
            block_time (float): 音频块处理时间(秒)，默认为0.25。
            threhold (int): 音量阈值(dB)，默认为-60。
            crossfade_time (float): 交叉淡入淡出时间(秒)，默认为0.05。
            extra_time (float): 额外处理时间(秒)，默认为2.5。
            I_noise_reduce (bool): 输入降噪开关，默认为False。
            O_noise_reduce (bool): 输出降噪开关，默认为False。
            use_pv (bool): 相位声码器开关，默认为False。
            rms_mix_rate (float): RMS混合比例，默认为0.0。
            index_rate (float): 索引混合比例，默认为0.0。
            n_cpu (int): CPU核心数限制，默认为min(n_cpu, 4)。
            f0method (str): F0提取方法("pm","harvest","crepe","rmvpe","fcpe")，默认为"fcpe"。
            sg_hostapi (str): 音频主机API名称，默认为空字符串。
            wasapi_exclusive (bool): WASAPI独占模式开关，默认为False。
            sg_input_device (str): 输入设备名称，默认为空字符串。
            sg_output_device (str): 输出设备名称，默认为空字符串。
        """
    ...
    class GUI:
        ...
        属性:
            gui_config (GUIConfig): GUI配置实例
            config (Config): 全局配置实例
            function (str): 当前功能模式
            delay_time (int): 延迟时间
            hostapis (list): 可用的音频主机API列表
            input_devices (list): 输入设备列表
            output_devices (list): 输出设备列表
            input_devices_indices (list): 输入设备索引列表
            output_devices_indices (list): 输出设备索引列表
            stream: 音频流对象
        """
        def __init__(self) -> None:
